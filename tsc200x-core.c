...
{
	if (pressure) {
		printk(KERN_ALERT "@@@@@ pressure to touchpanel \n");
		input_report_abs(ts->idev, ABS_X, x);
		input_report_abs(ts->idev, ABS_Y, y);
		input_report_abs(ts->idev, ABS_PRESSURE, pressure);
		if (!ts->pen_down) {
			printk(KERN_ALERT "@@@@@ pen down \n");	
...
static irqreturn_t tsc200x_irq_thread(int irq, void *_ts)
{
	...
	int error;
	printk(KERN_ALERT "@@@@@  irqreturn_t functions \n");	
...
static void tsc200x_penup_timer(unsigned long data)
{
	...
	unsigned long flags;
	printk(KERN_ALERT "@@@@@  tsc200x_penup_timer functions \n");	
	spin_lock_irqsave(&ts->lock, flags);
...
{	
	printk(KERN_ALERT "@@@@@   tsc200x_start functions \n");	
...
static void __tsc200x_disable(struct tsc200x *ts)
{
	...
	printk(KERN_ALERT "@@@@@   tsc200x_disable functions \n");
	enable_irq(ts->irq);
}
		  ...
		  int (*tsc200x_cmd)(struct device *dev, u8 cmd))
{
	...
	printk(KERN_ALERT "FONKSIYONA GIRIS YAPIYOR\n");
	struct tsc200x *ts;
	...
	printk(KERN_ALERT "VIO REGULATORU GECTI\n");
	if (!ts->reset_gpio && pdata)
		printk(KERN_ALERT "vio regulator reset\n");
